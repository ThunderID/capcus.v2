...
@if (!$widget_error_count)
	...
	@section('widget_body')
		<ul class="nav nav-pills nav-stacked">
			...
			<li role="presentation" class='{{ str_is("overview", $current_mode) ? "bg-light-blue" : "" }}'>
				<a href="{{route('admin.' . $route_name . '.edit', ['id' => $user->id])}}" class='text-black'>Edit <i class='fa fa-chevron-right pull-right text-xs pt-5'></i></a>
			</li>
			<li role="presentation" class='{{ str_is("overview", $current_mode) ? "bg-light-blue" : "" }}'>
				<a href="{{route('admin.' . $route_name . '.delete_confirmation', ['id' => $user->id])}}" class='text-black'>Delete <i class='fa fa-chevron-right pull-right text-xs pt-5'></i></a>
			</li>
		</ul>
	@overwrite
@else
