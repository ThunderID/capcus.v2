		...
		if ($headlines)
		{
			foreach ($headlines as $v)
			{
				$events[] = ['id' 	=> $v->id, 
							 ...
							 'url'	=> route('admin.'.$route_name.'.headlines.show', ['id' => $v->id]),
							 'class'=> 'event_info',
	...
	@section('js')
		...
		<script type="text/javascript">
			...
			$('#calendar .fc-right').append('<a href="{{ 
															route('admin.'.$route_name.'.headlines.index', [
														...
														'<a href="{{ route('admin.'.$route_name.'.headlines.index', ['filter_headline_month' => $filters['filter_headline_since']->addMonth(2)->month, 'filter_headline_year' => $filters['filter_headline_since']->year]) }}" class="btn btn-default btn-sm" style="width:40px;font-weight:bold;"><i class="fa fa-chevron-right"></i></a>' 
											);
