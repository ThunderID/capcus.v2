...
<table class="table table-hover tour_schedule_table table-sorter">
	...
	<tbody>
		@forelse ($tour_schedules as $schedule)
			...
			?>
			{{-- GENERATE CLASS --}}
			<tr class='text-regular {{ (!$schedule->still_active ? "text-muted":"") }} {{ $tr_class }}'
				...
				<td class='pt-md pb-lg text-left' data-sort-value="{{$schedule->tour->destinations[0]->long_name}}">
					...
					<div class='hidden-xs'>
						...
						<div class='mt-md'>					
							<a href="{{route('web.tour.show', ['travel_agent' => $schedule->tour->travel_agent->slug, 'tour_slug' => $schedule->tour->slug, 'schedule' => $schedule->departure->format('Ymd')])}}" class='btn btn-yellow'>Detail</a>
							<a href="javascript:;" data-id='{{$schedule->id}}' class='btn btn-yellow {{ in_array( $schedule->id, Session::get("compare_cart")) ? "btn-ningkan-active": ""}} compare_tour add'>
								<i class='glyphicon glyphicon-ok {{ in_array( $schedule->id, Session::get('compare_cart')) ? '': 'hidden'}}'></i> <span class='bandingkan_label'>Bandingkan</span>
							</a>
						</div>
					...
					<div class='hidden-sm hidden-md hidden-lg'>
						...
						<div class='mt-md text-center'>
							<a href="{{route('web.tour.show', ['travel_agent' => $schedule->tour->travel_agent->slug, 'tour_slug' => $schedule->tour->slug, 'schedule' => $schedule->departure->format('Ymd')])}}" class='btn btn-yellow'>Detail</a>
							<a href="javascript:;" data-id='{{$schedule->id}}' class='btn btn-yellow {{ in_array( $schedule->id, Session::get('compare_cart')) ? 'btn-bandingkan-active': ''}} compare_tour add'>
								<i class='glyphicon glyphicon-ok {{ in_array( $schedule->id, Session::get('compare_cart')) ? '': 'hidden'}}'></i> <span class='bandingkan_label'>Bandingkan</span>
							</a>
						</div>
					</div>
