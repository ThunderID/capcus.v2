		...
		<div class='content pt-sm'>
			<div class="row">
				<div class="col-xs-12 col-sm-12 col-md-10 col-lg-10">
					...
					<div class='hidden-xs hidden-sm compare_tour_list'>
						@forelse ($compare_cart as $k => $cart)
							<a href='javascript:;' class='btn btn-yellow compare_tour remove text-sm  mr-5 mb-xs ' data-id='{{$cart->id}}'><i class='glyphicon glyphicon-remove'></i> {{$cart->tour->name}} ({{$cart->tour->travel_agent->name}})</a>
						@empty
						@endforelse
					</div>
				...
				<div class="hidden-xs hidden-sm col-md-2 col-lg-2 tour_list">
					<a href='{{ route("web.tour.compare", ["id_list" => implode(',',$compare_cart->lists("id")->toArray())]) }}' class='pull-right btn btn-yellow pt-xl pb-xl mt-xs go-compare'>Bandingkan</a>
				</div>
				<div class="col-xs-12 col-sm-12 hidden-md hidden-lg text-center bg-black">
					<a href='{{ route("web.tour.compare", ["id_list" => implode(',',$compare_cart->lists("id")->toArray())]) }}' class='btn btn-yellow mt-xs mb-xs go-compare'>Bandingkan</a>
				</div>
			</div>
		</div>
	</div>
