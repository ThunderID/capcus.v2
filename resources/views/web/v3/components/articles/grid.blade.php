...
<div class="row">
	@forelse ($articles as $article)
		<div class="col-xs-{{$col_xs}} col-sm-{{$col_sm}} col-md-{{$col_md}} col-lg-{{$col_lg}} mb-lg">
			<div class='article grid relative'>
				<a href="{{ route('web.blog.show', ['year' => $article->published_at->year, 'month' => $article->published_at->month, 'slug' => $article->slug])}}">
					...
					<div class='label text-md text-light'>{{$article->title}}</div>
				</a>
			</div>
		</div>
	@empty
